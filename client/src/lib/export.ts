...
export async function exportToPDF(companyName: string): Promise<void> {
  ...
  pdf.save(`${companyName.replace(/\s+/g, "_")}_MC_Report.pdf`);
}
...
export async function exportToPPTX(report: ReportData): Promise<void> {
  ...
  cover.addText("Maudslay Consulting", { x: 0.4, y: 1.5, w: 12, h: 0.6, fontSize: 14, color: EMERALD, fontFace: "Arial" });
  ...
  cover.addText(`Generated ${formatDate(new Date())} · Maudslay Consulting Insight Generator`, {
  ...
  prs.writeFile({ fileName: `${report.companyName.replace(/\s+/g, "_")}_MC_Report.pptx` });
}
...
<style>
  ...
  h1 { font-size: 2.5rem; color: #CC0000; margin-bottom: 0.25rem; }
  h2 { font-size: 1.25rem; color: #CC0000; border-bottom: 1px solid #1e2d3d; padding-bottom: 8px; margin: 2rem 0 1rem; }
  ...
  .tag { display: inline-block; background: #3a0000; color: #ff6666; border-radius: 4px; padding: 2px 8px; font-size: 0.75rem; margin: 2px; }
...
<div class="container">
  <div style="margin-bottom:2rem">
    <div style="color:#475569;font-size:0.875rem;margin-bottom:4px">Maudslay Consulting Insight Generator</div>
  ...
  <h2>Financials</h2>
  <div class="grid">
    <div class="card"><div class="label">Revenue</div><div class="value">${report.financials.revenue}</div><div style="color:#CC0000;font-size:0.875rem">${report.financials.revenueGrowth}</div></div>
  ...
  <div class="footer">
    <p>Generated by Maudslay Consulting Insight Generator · ${formatDate(new Date())} · For internal use only.</p>
  ...
  a.href = url;
  a.download = `${report.companyName.replace(/\s+/g, "_")}_MC_Report.html`;
  a.click();
...
export async function exportInvestorPPTX(
  ...
  const addStyledSlide = (slideNum: number, title: string, content: (slide: ReturnType<typeof prs.addSlide>) => void) => {
    ...
    // Red accent line
    slide.addShape(prs.ShapeType.rect, { x: 0, y: 0.65, w: 13.33, h: 0.04, fill: { color: RED } });
    // Slide number
    slide.addText(String(slideNum), {
      x: 0.25, y: 0.12, w: 0.45, h: 0.4,
      fontSize: 11, bold: true, color: RED, fontFace: "Arial",
    });
    // Slide title
    slide.addText(title, {
    ...
    // Brand mark
    slide.addText("MC", {
      x: 12.1, y: 0.12, w: 1.0, h: 0.4,
      fontSize: 13, bold: true, color: RED, fontFace: "Arial", align: "right",
    });
    content(slide);
    return slide;
  };
  ...
  const cover = prs.addSlide();
  cover.background = { color: DARK };
  cover.addShape(prs.ShapeType.rect, { x: 0, y: 0, w: 0.1, h: 7.5, fill: { color: RED } });
  cover.addText("Maudslay Consulting", { x: 0.4, y: 1.5, w: 12, h: 0.5, fontSize: 13, color: RED, fontFace: "Arial" });
  ...
  cover.addText(`${presentation.date} · Generated by Maudslay Consulting Insight Generator`, {
    ...
    addStyledSlide(slide.slideNumber, slide.title, (s) => {
      ...
      if (slide.headline) {
        s.addText(slide.headline, {
          ...
          fontSize: 13, color: RED, fontFace: "Arial", italic: true,
        });
      }
      ...
      if (slide.metric?.value) {
        s.addShape(prs.ShapeType.rect, {
          ...
          fill: { color: "200000" }, line: { color: RED, width: 1 },
        });
